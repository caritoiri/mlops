...
# ──────────────────────────────────────────────────────────────────────────────
print("2) Creando/actualizando Data Asset heart-disease-csv…")
data_asset = Data(
    name="heart-disease-csv",
    description="CSV Heart Disease para clases",
    path="data/heart-disease.csv",  # ruta local; el SDK la sube al datastore del WS
    type="uri_file",
)
...
register_name = "heart-disease-logreg-mlflow"  # para el paso de train

# ──────────────────────────────────────────────────────────────────────────────
# 3) Definición del Pipeline (train -> score -> eval)
# ──────────────────────────────────────────────────────────────────────────────
print("3) Definiendo pipeline heart_disease_pipeline…")


@dsl.pipeline(
    compute=COMPUTE_NAME, description="Heart Disease pipeline: train -> score -> eval"
)
def heart_disease_pipeline(raw_data: Input):
    train_step = train_comp(raw_data=raw_data, register_name=register_name)
    score_step = score_comp(
        model_dir=train_step.outputs.model_dir, test_data=train_step.outputs.test_data
    )
...
pipeline_job: PipelineJob = heart_disease_pipeline(
    raw_data=Input(type="uri_file", path=data_ref)
)
...
pipeline_job.display_name = "Heart-Disease-MLflow-Pipeline"
pipeline_job.experiment_name = "heart-disease-mlflow"  # aparece en Experiments
pipeline_job.tags = {
    "course": "DML-004",
    "module": "MLOps",
